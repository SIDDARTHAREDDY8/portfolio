...
const SECTION_STATES: Record<string, KeyboardState> = {
  ...
  // Project 6 — right-aligned, keyboard left.
  project6: {
    yaw: 0.5,
    pitch: 0.8,
    roll: -0.3,
  ...
  // Project 7 — left-aligned, keyboard right.
  project7: {
    yaw: 0,
    pitch: 0.7,
    roll: 0.2,
...
// hover for variety. Raw ArrayBuffers are pre-fetched on mount so the network
// hop is already done by the time the user first hovers. Decoded into
// AudioBuffers the moment a context is available.
const KEY_SOUND_URLS = [
  "/sounds/switch_press.mp3",
  "/sounds/switch_release.mp3",
] as const;
const keySoundBuffers: (AudioBuffer | null)[] = [null, null];
// Raw bytes cached from the pre-fetch — slice(0) clones before decodeAudioData
// consumes the buffer.
const rawSoundData: (ArrayBuffer | null)[] = [null, null];
let rawSoundsPreloaded = false;
function preloadRawSounds() {
  if (rawSoundsPreloaded || typeof window === "undefined") return;
  rawSoundsPreloaded = true;
  KEY_SOUND_URLS.forEach((url, i) => {
    fetch(url)
      .then((r) => r.arrayBuffer())
      .then((buf) => { rawSoundData[i] = buf; })
      .catch(() => {});
  });
}
...
function loadKeySounds(ctx: AudioContext): Promise<void> {
  ...
  keySoundsLoading = Promise.all(
    KEY_SOUND_URLS.map((url, i) => {
      const decodeFrom = (buf: ArrayBuffer) =>
        ctx
          .decodeAudioData(buf.slice(0)) // slice clones so the buffer stays reusable
          .then((decoded) => { keySoundBuffers[i] = decoded; })
          .catch(() => {});
      // Use pre-fetched bytes if available — no extra network round-trip.
      if (rawSoundData[i]) return decodeFrom(rawSoundData[i]!);
      return fetch(url)
        .then((r) => r.arrayBuffer())
        .then(decodeFrom)
        .catch(() => {});
...
function playKeyClick(seed = 0) {
  ...
  try {
    ...
    // Previously this returned early when suspended, so the first real
    // gesture never played — sound was one hover behind. Now we resume
    // and play in the same microtask chain.
...
function Keyboard() {
  ...
  // Pre-fetch sound bytes on mount so they're ready to decode the moment the
  // AudioContext becomes available — eliminates the network lag on first hover.
  useEffect(() => { preloadRawSounds(); }, []);
